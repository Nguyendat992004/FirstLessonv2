...
public class Domain
{
    ...
    private List<Manga> ParseMangaList(XmlDocument doc)
    {
        ...
        for (int i = 0; i < nodes.Count; i++)
        {
            ...
            mangaList.Add(manga);
            Console.WriteLine($"{i} : Title = {title} Description = {description} Chapter = {lastChapter}{Environment.NewLine}" +
                              $" CoverUrl = {coverUrl},{Environment.NewLine} MangaUrl = {mangaUrl}");
        }
    ...
    private MangaList Parse(string html)
    {
        try
        {
            ...
            File.WriteAllText("docbefore.html", html); // hdcphuUpdated
